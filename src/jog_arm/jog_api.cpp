...
// Param linear_vel_scale: scales the velocity components of outgoing Twist
// msgs. Should be 0<linear_vel_scale<1
...
bool jog_api::jacobian_move(geometry_msgs::PoseStamped &target_pose,
  ...
  distance_and_twist = calc_distance_and_twist(current_pose, target_pose,
                                               linear_vel_scale, rot_vel_scale);

  ros::Time begin = ros::Time::now();

  // Is the current pose close enough?
  while ((distance_and_twist.translational_distance > trans_tolerance ||
    ...
    distance_and_twist = calc_distance_and_twist(
        current_pose, target_pose, linear_vel_scale, rot_vel_scale);
...
////////////////////////////////////
bool jog_api::transform_a_pose(geometry_msgs::PoseStamped &pose,
  ...
  geometry_msgs::TransformStamped current_frame_to_target =
      tf_buffer_.lookupTransform(pose.header.frame_id, desired_frame,
...
// distance_and_twist.translational_distance holds the linear distance to target
// pose
// distance_and_twist.rotational_distance holds the rotational distance to
// target pose
...
////////////////////////////////////////////////////////////////////////////////
jog_api::distance_and_twist
jog_api::calc_distance_and_twist(const geometry_msgs::PoseStamped &current_pose,
  ...
  ///////////////////////////////////////////////////////////////////////////////
  // Normalize the twist to the target pose. Calculate distance while we're at
  // it
  ///////////////////////////////////////////////////////////////////////////////
  // Linear:
  ...
  result.twist.twist.linear.x = linear_vel_scale * result.twist.twist.linear.x /
                                result.translational_distance;
  result.twist.twist.linear.y = linear_vel_scale * result.twist.twist.linear.y /
                                result.translational_distance;
  result.twist.twist.linear.z = linear_vel_scale * result.twist.twist.linear.z /
                                result.translational_distance;
  ...
  result.twist.twist.angular.y =
      rot_vel_scale * result.twist.twist.angular.y / result.rotational_distance;
  result.twist.twist.angular.z =
      rot_vel_scale * result.twist.twist.angular.z / result.rotational_distance;
